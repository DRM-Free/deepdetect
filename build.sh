...
cpu_build() {
    ...
    "armv7")
        cmake .. -DUSE_NCNN=ON -DRPI3=ON -DUSE_HDF5=OFF -DUSE_TORCH=OFF -DBUILD_OPENCV=${BUILD_OPENCV} -DRELEASE=${DEEPDETECT_RELEASE}
        make -j6
        ;;

    *)
        cmake .. -DUSE_XGBOOST=ON -DUSE_CAFFE=ON -DUSE_CPU_ONLY=ON -DUSE_SIMSEARCH=ON -DUSE_TSNE=ON -DUSE_NCNN=ON -DBUILD_OPENCV=${BUILD_OPENCV} -DRELEASE=${DEEPDETECT_RELEASE}
        make -j6
        ;;
    esac
